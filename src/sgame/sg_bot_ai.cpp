...
Cvar::Cvar<bool> g_bot_buildAliens("g_bot_buildAliens", "whether alien bots should build", Cvar::NONE, true);
Cvar::Cvar<bool> g_bot_buildHumans("g_bot_buildHumans", "whether human bots should build", Cvar::NONE, true);
...
buildable_t BotChooseBuildableToBuild( gentity_t *self )
{
	...
	if ( G_Team( self ) == TEAM_HUMANS )
	{
		...
		else if ( !G_IsSuddenDeath() )
		{
			...
			int r = rand();
			if ( BG_BuildableUnlocked( BA_H_ROCKETPOD ) && r < RAND_MAX * g_bot_buildProbRocketPod.Get() )
			{
